...
internal sealed class LauncherForm : Form
{
    ...
    private static LauncherSettings LoadSettings()
    {
        ...
        {
            // Invalid settings fall back to the known local deployment paths.
        }
        return new LauncherSettings();
    }
    ...
    private LauncherModelCatalog LoadModelCatalog()
    {
        try
        {
            ...
            {
                var loaded = JsonSerializer.Deserialize<LauncherModelCatalog>(File.ReadAllText(ModelCatalogPath));
                if (loaded is { Models.Count: > 0 })
                {
                    return loaded;
                }
            }
        ...
        {
            // Recreate defaults when the catalog cannot be parsed.
        }
        return CreateDefaultModelCatalog();
    }
    ...
    private static void SaveUpdateState(GitHubUpdateSource source, string commitSha)
    {
    ...
    private static async Task<SourceUpdateCheck> FetchUpdateCheckAsync(GitHubUpdateSource source)
    {
        string sha;
        DateTimeOffset date;
        string message;
        try
        {
            using var response = await GitHubClient.GetAsync($"https://api.github.com/repos/{source.Repository}/commits/{source.Branch}");
        ...
        catch (Exception restException)
        {
            try
            {
                (sha, date, message) = await FetchUpdateCheckFromAtomAsync(source);
            }
            catch (Exception atomException)
            {
                throw new InvalidOperationException(
                    $"GitHub REST API and Atom feed both failed for {source.Repository}. " +
                    $"REST: {restException.Message} Atom: {atomException.Message}", atomException);
            }
        }

        var state = LoadUpdateState(source);
        return new SourceUpdateCheck(source, sha, date, message, state?.CommitSha != sha, state is not null);
    }

    private static async Task<(string Sha, DateTimeOffset Date, string Message)> FetchUpdateCheckFromAtomAsync(GitHubUpdateSource source)
    {
        using var response = await GitHubClient.GetAsync($"https://github.com/{source.Repository}/commits/{source.Branch}.atom");
        response.EnsureSuccessStatusCode();
        await using var stream = await response.Content.ReadAsStreamAsync();
        ...
        var entry = feed.Root?.Element(atom + "entry")
            ?? throw new InvalidDataException("GitHub Atom feed did not contain a commit entry.");
        var id = entry.Element(atom + "id")?.Value;
        var sha = id?.Split('/').LastOrDefault();
    ...
    private static async Task<string[]> GetChangedDependencyFilesAsync(GitHubUpdateSource source)
    {
        ...
        foreach (var relativePath in source.DependencyFiles)
        {
            try
            {
                var remote = await GitHubClient.GetByteArrayAsync($"https://raw.githubusercontent.com/{source.Repository}/{source.Branch}/{relativePath}");
            ...
            {
                // A missing optional dependency file should not block update preview.
            }
        }
    ...
    private void StopProcesses(IReadOnlyCollection<int> pids)
    {
        ...
        foreach (var pid in pids.Distinct())
        {
            ...
            {
                AppendLog($"停止 PID {pid} 失败：{ex.Message}");
            }
        }

        AppendLog(stopped.Count > 0
            ? $"已停止已识别 AI 服务进程：{string.Join(", ", stopped)}"
            : "未能停止任何服务进程。");
    }
