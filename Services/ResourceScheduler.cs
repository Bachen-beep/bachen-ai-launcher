...
internal static class SystemResourceProbe
{
    ...
    {
        var gpu = ReadGpuMemory();
        var memory = ReadSystemMemory();
        return new SystemResourceSnapshot(gpu?.UsedMiB, gpu?.TotalMiB, memory.AvailableMiB, memory.TotalMiB);
    }
    ...
    public static (int UsedMiB, int TotalMiB)? ReadGpuMemory()
    {
        try
        {
            ...
            return values is { Length: >= 2 } && int.TryParse(values[0], out var used) && int.TryParse(values[1], out var total)
                ? (used, total)
                : null;
        }
        catch
